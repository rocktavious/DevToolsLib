...
class Settings(JsonDocument):
    ...
    def __init__(self):
        ...
        if sys.platform == 'darwin' :
            self._appdata = Path(os.path.join(os.environ['HOME'], 'Documents', default_user_settings.COMPANY, __pkgname__))
        else :
            self._appdata = Path(os.path.join(os.environ['APPDATA'], default_user_settings.COMPANY, __pkgname__))
